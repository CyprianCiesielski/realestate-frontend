...
import { type UserAuth, type LoginResponse } from '../features/auth/types.ts';
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const handleToken = (token: string) => {
        try {
            ...
            setUser({ ...decoded, initial: decoded.sub ? decoded.sub[0].toUpperCase() : '?', role: decoded.role || 
                    (decoded as any).authorities?.[0] });
